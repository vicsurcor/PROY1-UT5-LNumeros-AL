...
//        }
    ...
    public int segundoMaximo() {
       //TODO
        ...
        for (int i = 0; i < pos; i++){
            if (lista[i] > max){ //Es un nuevo primer máximo
                max2 = max; //El segundo máximo pasa a ser el antiguo primer máximo
                max = lista[i];//Actualizo el nuevo primer máximo
            }
            else if (lista[i]> max2 && lista[i]<max) {//Si el número está entre max2 y max
                max2 = lista[i];//Sólo actualizamos el segundo máximo
            }
        }
        return max2;
    }
